...
const QuizOption = ({ icon, title, onClick }) => {
  return (
    <button
      onClick={onClick}
      className='border-2 border-gray-500 rounded-lg bg-gray-100 w-40 h-40 flex flex-col justify-center items-center cursor-pointer hover:bg-gray-300'
    >
      {icon}
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription className='pb-6'>
            Selamat, kamu telah menyelesaikan cerita. Silahkan pilih jenis
            permainan
          </DialogDescription>
          <div className='flex justify-evenly items-center pb-6 gap-10'>
            <QuizOption
              onClick={onClickMultipleChoice}
              icon={<ArrowDownAZ className='opacity-80' size={100} />}
              title='Pilihan Ganda'
            />
            <QuizOption
              onClick={onClickWordCompletion}
              icon={<ClipboardPen className='opacity-80' size={100} />}
              title='Melengkapi Kata'
            />
          </div>
